...
def dashboard():
    ...
    if conn:
        ...
        # Format the created_at field for each notice to Indian Standard Time
        ist = pytz.timezone('Asia/Kolkata')  # Set timezone to IST
        formatted_notices = []  # Create a list to hold formatted notices

        for notice in notices:
            # Convert created_at to IST
            utc_time = datetime.strptime(notice['created_at'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=pytz.utc)
            notice_created_at_ist = utc_time.astimezone(ist).strftime('%B %d, %Y, %I:%M %p')
            # Create a new dictionary for the formatted notice
            formatted_notice = {
                'id': notice['id'],
                'title': notice['title'],
                'content': notice['content'],
                'created_at': notice_created_at_ist  # Add the formatted date
            }
            formatted_notices.append(formatted_notice)  # Add to the list of formatted notices

        conn.close()
        return render_template('dashboard.html', role=role, pending_users=pending_users, notices=formatted_notices)
    else:
        return 'Database connection error!'
...
def upload_timetable():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            # Save the file directly in the 'static' directory
            directory = 'static'  # Directly saving in static folder
            ...
            # Save the relative file path (without 'timetables/') in the database
            conn.execute(
                'INSERT INTO timetables (file_path, uploaded_by) VALUES (?, ?)',
                (file_name, session['username'])  # Save file name without subdirectory
            )
            conn.commit()
    ...
    return render_template('upload_timetable.html', timetables=timetables)





@app.route('/delete_timetable_file/<int:timetable_id>', methods=['POST'])
def delete_timetable_file(timetable_id):
    if 'username' not in session or session['role'] != 'admin':
        return redirect(url_for('login'))
    ...
    if timetable:
        ...
        file_path = f'static/{timetable["file_path"]}'
...
# Route for the admin to create a new notice
...
def delete_notice(notice_id):
    ...
    conn = get_db_connection()
    if conn:
        # Delete the notice with the given notice_id
